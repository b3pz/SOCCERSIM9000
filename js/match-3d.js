...
function updateBroadcastHud(){
 ...
 // FIX 2026-09 (47): logo della competizione in corso al posto della
 // scritta fissa "SERIEA 9000 SIM" - stesso bollino usato come pulsante
 // nel carosello Coppe e Tornei (S9V10.logoForCompetition).
 const wmImg=broadcastHud.querySelector('.s9-tv-watermark-logo');
 if(wmImg){
  const compKey=(typeof S9Competition!=='undefined'&&S9Competition.key)?S9Competition.key():'seriea';
  if(wmImg.dataset.comp!==compKey){
   wmImg.dataset.comp=compKey;
   wmImg.src=window.S9V10?.logoForCompetition?window.S9V10.logoForCompetition(compKey):'assets/logo/seriea9000_logo.png';
  }
 }
 ...
 if(cbox){
  ...
  cbox.innerHTML=lines.map(l=>{
   ...
   return m?`<p>${S9Icon('mic')} <b>${m[1]}:</b> ${m[2]}</p>`:`<p>${S9Icon('mic')} ${raw}</p>`;
  }).join('');
 }
}
...
function installBroadcastLayer(){
 ...
 broadcastHud=document.createElement('div');broadcastHud.className='s9-tv-layer';broadcastHud.innerHTML=`<div class="s9-tv-scorebug"><div class="s9-tv-channel">${current?.channel||(window.S9Channel?window.S9Channel():'S9 90')} <i>LIVE</i></div><div class="s9-tv-clock">1T&nbsp;&nbsp;00'</div><div class="s9-tv-team"><img class="s9-tv-home-crest" alt=""><span class="s9-tv-home-name">CASA</span><b class="s9-tv-home-score">0</b></div><div class="s9-tv-team"><img class="s9-tv-away-crest" alt=""><span class="s9-tv-away-name">OSPITI</span><b class="s9-tv-away-score">0</b></div></div><div class="s9-tv-watermark"><img class="s9-tv-watermark-logo" alt="SerieA 9000 SIM"></div><div class="s9-tv-event" hidden><b class="s9-tv-event-minute">00'</b><span class="s9-tv-event-text"></span></div><div class="s9-tv-goal" hidden aria-live="polite"><small>GOL</small><strong>MARCATORE</strong><span></span></div><div class="s9-tv-commentary" hidden aria-live="polite"></div><div class="s9-tv-bug-corner" aria-hidden="true"></div><div class="s9-tv-controls" aria-label="Comandi partita"><button type="button" class="s9-tv-pause" aria-label="Metti in pausa la partita">Ⅱ</button><button type="button" class="s9-tv-speed" aria-label="Cambia velocità">1×</button><button type="button" class="s9-tv-incita" aria-label="Incita la squadra">${S9Icon('megaphone')} 2/2</button><button type="button" class="s9-tv-tactics" aria-label="Apri tattica e cambi">${S9Icon('gear')}</button></div><button type="button" class="s9-tv-exit" aria-label="Esci dalla modalità televisiva">${S9Icon('close')}</button>`;
 /* FIX 2026-09 (5): "INCITA LA SQUADRA" viveva solo nella toolbar sotto al
    ...
    del tutto (fullscreen nasconde tutto cio' che non e' dentro l'elemento
    stesso), esattamente come gia' capitava a suo tempo per le tattiche
    (da cui parkTacticsForBroadcast/openBroadcastTactics qui sotto). Stesso
    rimedio: un pulsante equivalente dentro l'overlay TV, che richiama la
    stessa funzione globale applyIncita() gia' usata dal pulsante normale. */
 document.getElementById('pitch').appendChild(broadcastHud);broadcastHud.querySelector('.s9-tv-exit').onclick=toggleFullscreen;broadcastHud.querySelector('.s9-tv-pause').onclick=toggleBroadcastPause;broadcastHud.querySelector('.s9-tv-speed').onclick=cycleBroadcastSpeed;broadcastHud.querySelector('.s9-tv-incita').onclick=()=>{window.applyIncita?.(window.mySide?.()||'h');window.updateIncitaButton?.()};broadcastHud.querySelector('.s9-tv-tactics').onclick=openBroadcastTactics;updateBroadcastHud();window.updateIncitaButton?.();
}
